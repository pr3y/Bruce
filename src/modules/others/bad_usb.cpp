...
NewScript:
    ...
    if (fs != nullptr) {
        ...
        tft.fillScreen(bruceConfig.bgColor);
        if (first_time) {
            auto createKeyboardSetter = [=](const uint8_t *layout) { return [=]() { chooseKb(layout); }; };
            options = {
    ...
    while (1) {
        ...
        if (key.pressed && (millis() - debounce > 200)) {
            ...
            if (key.enter) Kb.println();
                ...
                for (char k : key.word) { Kb.press(k); }
                for (auto k : key.modifier_keys) { Kb.press(k); }
            }
