...
void moveShark() {
    ...
    #if defined(ARDUINO_M5STICK_CPLUS) || defined(ARDUINO_M5STICK_CPLUS2) || defined(M5STACK) || defined(CYD) // checkEscPress is the same of checkPrevPress in these devices
    if (checkSelPress())
    #else
    if (checkPrevPress())
    #endif
