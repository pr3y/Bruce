...
void IRAM_ATTR audioSampleTimer() {
    audio_frame_idx++;
    if (audio_frame_idx >= DEFAULT_FRAGSIZE) { audio_frame_idx = 0; }

    uint16_t s = audio_frame[audio_frame_idx];
    LEDC.channel_group[0].channel[2].duty.duty = s >> 3;
    LEDC.channel_group[0].channel[2].conf0.sig_out_en =
        1; // This is the output enable control bit for channel
    LEDC.channel_group[0].channel[2].conf1.duty_start =
        1; // When duty_num duty_cycle and duty_scale has been configured. these register won't take effect
           // until set duty_start. this bit is automatically cleared by hardware
