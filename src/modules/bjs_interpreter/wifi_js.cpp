...
duk_ret_t native_wifiConnect(duk_context *ctx) {
  ...
  if (duk_is_number(ctx, 1))
    timeout_in_seconds = duk_to_int(ctx, 1);
...
const char *wifi_enc_types[] = {
    ...
    "WPA_WPA2_PSK",  "ENTERPRISE", "WPA2_ENTERPRISE", "WPA3_PSK",
    "WPA2_WPA3_PSK", "WAPI_PSK",   "WPA3_ENT_192",    "MAX"};
...
duk_ret_t native_wifiScan(duk_context *ctx) {
  ...
  for (int i = 0; i < nets; i++) {
    ...
    const char *enctype =
        enctypeInt < 12 ? wifi_enc_types[enctypeInt] : "UNKNOWN";
    ...
    bduk_put_prop(ctx, obj_idx, "MAC", duk_push_string,
...
duk_ret_t native_httpFetch(duk_context *ctx) {
  ...
  if (duk_is_object(ctx, 1)) {
    if (duk_get_prop_string(ctx, 1, "body")) {
      ...
      if (arg1Type & (DUK_TYPE_MASK_STRING | DUK_TYPE_MASK_NUMBER |
        ...
        duk_push_string(ctx,
                        "stringify"); /* -> [ global Object "stringify" ] */
    ...
    if (duk_get_prop_string(ctx, 1, "responseType")) {
      const char *returnResponseTypeString =
          duk_get_string_default(ctx, -1, "string");
  ...
  int httpResponseCode =
      http.sendRequest(requestType, (uint8_t *)bodyRequest, bodyRequestLength);
  ...
  for (size_t i = 0; i < http.headers(); i++) {
    bduk_put_prop(ctx, headersObjectIdx, http.headerName(i).c_str(),
                  duk_push_string, http.header(i).c_str());
  }
  ...
  if (!isChunked) {
    payloadSize = contentLength < 1 ? (psramFoundValue ? 16384 : 4096)
                                    : contentLength + 1;
    payload = (char *)duk_push_fixed_buffer(ctx, payloadSize);

    if (payload == NULL) {
      return duk_error(ctx, DUK_ERR_ERROR, "%s: Memory allocation failed!",
                       "httpFetch");
    }
  }
  ...
  while (http.connected()) {
    ...
    if (isChunked) { // if header Transfer-Encoding: chunked
      ...
      stream->read(); // Consume '\n'
      int chunkSize =
          strtol(chunkSizeStr.c_str(), NULL, 16); // Convert hex to int
      if (chunkSize == 0)
        break; // Last chunk
      ...
      if (streamSize > 0) {
        ...
        if ((bytesRead + toRead + 1) > payloadSize)
          break;
  ...
  bduk_put_prop(ctx, obj_idx, "ok", duk_push_boolean,
