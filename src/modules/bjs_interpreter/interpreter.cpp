...
static duk_ret_t native_assert(duk_context *ctx) {
  ...
  return duk_error(ctx, DUK_ERR_ERROR, "Assertion failed: %s",
...
static duk_ret_t native_ledcSetup(duk_context *ctx) {
  int val =
      ledcSetup(duk_get_int(ctx, 0), duk_get_int(ctx, 1), duk_get_int(ctx, 2));
  duk_push_int(ctx, val);
  return 1;
}
...
static duk_ret_t native_pinMode(duk_context *ctx) {
  ...
  if (pin == 255) {
    return duk_error(ctx, DUK_ERR_TYPE_ERROR, "%s invalid %d argument: %s",
...
static duk_ret_t native_to_hex_string(duk_context *ctx) {
  ...
  if (arg0Type &
      (DUK_TYPE_MASK_STRING | DUK_TYPE_MASK_NUMBER | DUK_TYPE_MASK_BOOLEAN)) {
...
static duk_ret_t native_getDeviceName(duk_context *ctx) {
  const char *deviceName = bruceConfig.wifiAp.ssid != NULL
                               ? bruceConfig.wifiAp.ssid.c_str()
                               : "Bruce";
...
static duk_ret_t native_getFreeHeapSize(duk_context *ctx) {
  ...
  bduk_put_prop(ctx, obj_idx, "ram_min_free", duk_push_uint,
                info.minimum_free_bytes);
  bduk_put_prop(ctx, obj_idx, "ram_largest_free_block", duk_push_uint,
...
static duk_ret_t native_getPrevPress(duk_context *ctx) {
  if ((!duk_get_boolean_default(ctx, 0, 0) && check(PrevPress)) || PrevPress)
    duk_push_boolean(ctx, true);
  else
...
static duk_ret_t native_getSelPress(duk_context *ctx) {
  if ((!duk_get_boolean_default(ctx, 0, 0) && check(SelPress)) || SelPress)
    duk_push_boolean(ctx, true);
  else
    duk_push_boolean(ctx, false);
  return 1;
...
static duk_ret_t native_getAnyPress(duk_context *ctx) {
  if ((!duk_get_boolean_default(ctx, 0, 0) && check(AnyKeyPress)) ||
      AnyKeyPress)
    duk_push_boolean(ctx, true);
  else
    duk_push_boolean(ctx, false);
...
static duk_ret_t native_getKeysPressed(duk_context *ctx) {
  ...
  if (!key.pressed)
    return 1; // if nothing has beed pressed, return 1
  int arrayIndex = 0;
  for (auto i : key.word) {
    char str[2] = {i, '\0'};
...
#if defined(BUZZ_PIN)
  tone(BUZZ_PIN, duk_get_uint_default(ctx, 0, 500),
       duk_get_uint_default(ctx, 1, 1000));
#elif defined(HAS_NS4168_SPKR)
  //  alt. implementation using the speaker
  if (!duk_get_int_default(ctx, 2, 0)) {
    serialCli.parse("tone " + String(duk_to_int(ctx, 0)) + " " +
                    String(duk_to_int(ctx, 1)));
  }
#endif
  return 0;
}
...
static duk_ret_t native_subghzTransmitFile(duk_context *ctx) {
  ...
  // returns: bool==true on success, false on any error
  bool r =
      serialCli.parse("subghz tx_from_file " + String(duk_to_string(ctx, 0)));
  duk_push_boolean(ctx, r);
  return 1;
}
...
static duk_ret_t native_badusbRunFile(duk_context *ctx) {
  ...
  // MEMO: no need to check for board support (done in serialCli.parse)
  bool r =
      serialCli.parse("badusb tx_from_file " + String(duk_to_string(ctx, 0)));
  duk_push_boolean(ctx, r);
  return 1;
}
...
static duk_ret_t native_notifyBlink(duk_context *ctx) {
  ...
  uint8_t delayMsArg =
      (arg1Type & (DUK_TYPE_NONE | DUK_TYPE_UNDEFINED)) ? 0 : 1;
...
static duk_ret_t native_storageWrite(duk_context *ctx) {
  ...
  if (duk_is_number(ctx, 3)) {
    ...
    size_t fileSize = 0;
    char *fileContent =
        readBigFile(*fileParams.fs, fileParams.path, false, &fileSize);
...
static duk_ret_t native_storageRename(duk_context *ctx) {
  ...
  if (!oldFileParams.path.startsWith("/"))
    oldFileParams.path = "/" + oldFileParams.path;
...
static duk_ret_t native_require(duk_context *ctx) {
  ...
  if (filepath == "audio") {
    ...
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "release", native_badusbRelease, 1,
                              0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "releaseAll",
                              native_badusbReleaseAll, 0, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "print", native_badusbPrint, 1, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "println", native_badusbPrintln, 1,
                              0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "pressRaw", native_badusbPressRaw,
                              1, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "runFile", native_badusbRunFile, 1,
                              0);
    ...
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "message", native_dialogMessage, 2,
                              0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "info", native_dialogNotification,
                              2, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "success",
                              native_dialogNotification, 2, 1);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "warning",
                              native_dialogNotification, 2, 2);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "error", native_dialogNotification,
                              2, 3);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "choice", native_dialogChoice, 1,
                              0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "prompt", native_keyboard, 3, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "pickFile", native_dialogPickFile,
                              2, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "viewFile", native_dialogViewFile,
                              1, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "viewText", native_dialogViewText,
                              2, 0);
    ...
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "getName", native_getDeviceName, 0,
                              0);
    ...
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "getBatteryCharge",
                              native_getBattery, 0, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "getFreeHeapSize",
                              native_getFreeHeapSize, 0, 0);

  } else if (filepath == "gpio") {
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "pinMode", native_pinMode, 3, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "digitalRead", native_digitalRead,
                              1, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "analogRead", native_analogRead, 1,
                              0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "touchRead", native_touchRead, 1,
                              0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "digitalWrite", native_digitalWrite,
                              2, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "analogWrite", native_analogWrite,
                              2, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "dacWrite", native_dacWrite, 2,
                              0); // only pins 25 and 26
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "ledcSetup", native_ledcSetup, 3,
                              0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "ledcAttachPin",
                              native_ledcAttachPin, 2, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "ledcWrite", native_ledcWrite, 2,
                              0);
    ...
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "getKeysPressed",
                              native_getKeysPressed, 0,
                              0); // keyboard btns for cardputer (entry)
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "getPrevPress", native_getPrevPress,
                              1, 0);
    ...
    bduk_put_prop_c_lightfunc(ctx, idx_top, "is_equal", native_math_is_equal, 3,
                              0);
    ...
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "print", native_serialPrint,
                              DUK_VARARGS, 0);
    ...
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "readdir", native_storageReaddir,
                              1);
    ...
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "setFrequency",
                              native_subghzSetFrequency, 1, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "read", native_subghzRead, 0, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "readRaw", native_subghzReadRaw, 0,
                              0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "transmitFile",
                              native_subghzTransmitFile, 1, 0);
    ...
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "connected", native_wifiConnected,
                              0, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "connect", native_wifiConnect, 3,
                              0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "connectDialog",
                              native_wifiConnectDialog, 0, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "disconnect", native_wifiDisconnect,
                              0, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "scan", native_wifiScan, 0, 0);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "httpFetch", native_httpFetch, 2,
                              0);
...
static void registerConsole(duk_context *ctx) {
  duk_idx_t obj_idx = duk_push_object(ctx);
  bduk_put_prop_c_lightfunc(ctx, obj_idx, "log", native_serialPrintln,
                            DUK_VARARGS);
  bduk_put_prop_c_lightfunc(ctx, obj_idx, "debug", native_serialPrintln,
                            DUK_VARARGS, 2);
  bduk_put_prop_c_lightfunc(ctx, obj_idx, "warn", native_serialPrintln,
                            DUK_VARARGS, 3);
  bduk_put_prop_c_lightfunc(ctx, obj_idx, "error", native_serialPrintln,
                            DUK_VARARGS, 4);
...
void interpreterHandler(void *pvParameters) {
  log_d("init interpreter:\nPSRAM: [Free: %d, max alloc: %d],\nRAM: [Free: %d, "
        "max alloc: %d]\n",
        ESP.getFreePsram(), ESP.getMaxAllocPsram(), ESP.getFreeHeap(),
        ESP.getMaxAllocHeap());
  if (script == NULL) {
    return;
  }
  ...
  duk_context *ctx =
      duk_create_heap(alloc_function, realloc_function, free_function, NULL,
  ...
  if (scriptDirpath == NULL || scriptName == NULL) {
    ...
    bduk_register_string(ctx, "__filepath",
                         (String(scriptDirpath) + String(scriptName)).c_str());
  ...
  bduk_register_c_lightfunc(ctx, "wifiConnectDialog", native_wifiConnectDialog,
                            0);
  ...
  // Input
  bduk_register_c_lightfunc(ctx, "getKeysPressed", native_getKeysPressed,
                            0); // keyboard btns for cardputer (entry)
  ...
  bduk_register_c_lightfunc(ctx, "badusbReleaseAll", native_badusbReleaseAll,
                            0);
  ...
  bduk_register_c_lightfunc(ctx, "subghzSetFrequency",
                            native_subghzSetFrequency, 1);
  bduk_register_c_lightfunc(ctx, "subghzTransmitFile",
                            native_subghzTransmitFile, 1);
  // bduk_register_c_lightfunc(ctx, "subghzSetIdle", native_subghzSetIdle, 1);
  // TODO: subghzTransmit(string)

  // Dialog functions
  bduk_register_c_lightfunc(ctx, "dialogMessage", native_dialogNotification, 2,
                            0);
  bduk_register_c_lightfunc(ctx, "dialogError", native_dialogNotification, 2,
                            3);
  ...
  if (duk_peval_string(ctx, script) != DUK_EXEC_SUCCESS) {
    ...
    if (lineIndexOf != -1) {
      lineIndexOf += 5;
      errorLine = errorMessage.substring(
          lineIndexOf, errorMessage.indexOf("\n", lineIndexOf));
    } else if (evalIndexOf != -1) {
      evalIndexOf += 6;
      errorLine = errorMessage.substring(
          evalIndexOf, errorMessage.indexOf(")", evalIndexOf));
    }
