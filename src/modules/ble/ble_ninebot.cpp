...
static const uint8_t max2Gpayload[] = {0x55,0xAB,0x4D,0x41,0x58,0x32,0x53,0x63,0x6F,0x6F,0x74,0x65,0x72,0x5F,0x31};
static const uint8_t f2payload[]   = {0x55,0xAB,0x46,0x32,0x53,0x63,0x6F,0x6F,0x74,0x65,0x72,0x5F,0x31};
...
struct PayloadDef { const char* label; const uint8_t* data; size_t len; };
...
static std::vector<Option> buildModelOptions(NimBLERemoteCharacteristic* pTXChar, std::vector<Option>& deviceSelection) {
...
void BLENinebot::loop() {
    ...
    while (!check(EscPress)) {
        ...
        for (int i = 0; i < results.getCount(); ++i) {
            ...
            String name = adv.getName().length() ? String(adv.getName().c_str()) : String(adv.getAddress().toString().c_str());

            deviceSelection.push_back({name, [&, adv](void) mutable {
                redrawMainBorder();
