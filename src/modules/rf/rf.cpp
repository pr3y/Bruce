    ...
    while (rb) {
        ...
        if (item != nullptr) {
            if (rx_size != 0) {
                ...
                tft.fillRect(0, 20, TFT_WIDTH, TFT_HEIGHT, bruceConfig.bgColor);
                // Draw waveform based on signal strength
                for (size_t i = 0; i < rx_size; i++) {
                    int lineHeight = map(item[i].duration0 + item[i].duration1, 0, SIGNAL_STRENGTH_THRESHOLD, 0, TFT_HEIGHT/2);
                    int lineX = map(i, 0, rx_size - 1, 0, TFT_WIDTH - 1); // Map i to within the display width
                    // Ensure drawing coordinates stay within the box bounds
                    int startY = constrain(20 + TFT_HEIGHT / 2 - lineHeight / 2, 20, 20 + TFT_HEIGHT);
                    int endY = constrain(20 + TFT_HEIGHT / 2 + lineHeight / 2, 20, 20 + TFT_HEIGHT);
    ...
    while (1) {
        if (rcswitch.RAWavailable()) {
                ...
                for (int i = 0; i < RCSWITCH_RAW_MAX_CHANGES-1; i+=2) {
                    if(raw[i]==0) break;
                    #define TIME_DIVIDER TFT_WIDTH/8
                    if(raw[i]>20000) raw[i]=20000;
                    if(raw[i+1]>20000) raw[i+1]=20000;
                    if(line_w+(raw[i]+raw[i+1])/TIME_DIVIDER>TFT_WIDTH) { line_w=10; line_h+=10; }
                    if(line_h>TFT_HEIGHT) {
                        line_h = 15;
                        tft.fillRect(0, 12, TFT_WIDTH, TFT_HEIGHT, bruceConfig.bgColor);
                    }
