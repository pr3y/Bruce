...
void arpSpoofing(const Host& host, bool mitm) {
  ...
  while(!checkAnyKeyPress()) {
    if(tmp+2000<millis()){  // sends frames every 2 seconds
      ...
      tmp=millis();
      count++;
      tft.drawRightString("Spoofed " + String(count) + " times",TFT_WIDTH-12,TFT_HEIGHT-16,1);
    }
  }
  ...
  while(!checkAnyKeyPress()) {
    if(tmp+5000<millis()){  // sends frames every 5 seconds
      ...
      for(int i=1;i<255;i++) {
        ...
        delay(10);
        tft.drawRightString("   " + String(victimIP[0]) + "." + String(victimIP[1]) + "." + String(victimIP[2]) + "." + String(i), TFT_WIDTH-12,TFT_HEIGHT-16,1);
      }
      tmp=millis();
      tft.drawRightString("     Waiting...",TFT_WIDTH-12,TFT_HEIGHT-16,1);
    }
  }
  pcapFile.close();
}
...
void stationDeauth(Host host) {
  ...
  while(!checkAnyKeyPress()) {
    ...
    delay(50);
    if(millis()-tmp>1000) {
      tft.drawRightString(String(cont) + " fps", TFT_WIDTH-12,TFT_HEIGHT-16,1);
      cont=0;
      tmp=millis();
    }
  }
