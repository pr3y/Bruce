...
void fm_banner() {
  ...
  tft.setCursor(10, 10);
  tft.drawCentreString("~== Bruce Radio ==~", TFT_WIDTH/2, 10, SMOOTH_FONT);
  delay(500);
}
  ...
  if (!returnToMenu) {
    ...
    while (!checkEscPress() && !checkSelPress()) {
      ...
      if (noise_level != 0) {
        ...
        tft.fillRect(0, 40, TFT_WIDTH, TFT_HEIGHT, TFT_BLACK);
        // Draw waveform based on signal strength
        for (size_t i = 0; i < noise_level; i++) {
          int lineHeight = map(noise_level, 0, SIGNAL_STRENGTH_THRESHOLD, 0, TFT_HEIGHT/2);
          int lineX = map(i, 0, noise_level - 1, 0, TFT_WIDTH - 1); // Map i to within the display width
