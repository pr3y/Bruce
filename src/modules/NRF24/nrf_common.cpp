...
bool nrf_start() {
    ...
    if (bruceConfig.NRF24_bus.mosi == (gpio_num_t)TFT_MOSI &&
        bruceConfig.NRF24_bus.mosi != GPIO_NUM_NC) { // (T_EMBED), CORE2 and others
    ...
    } else if (bruceConfig.NRF24_bus.mosi ==
               bruceConfig.SDCARD_bus.mosi) { // CC1101 shares SPI with SDCard (Cardputer and CYDs)
        Serial.println(
            "Using this one!!!!!!!!!!!! --    --------- ------------ ------------ ---------- --------- ----"
        );
        NRFSPI = &sdcardSPI;
    } else if (bruceConfig.NRF24_bus.mosi == bruceConfig.CC1101_bus.mosi &&
               bruceConfig.NRF24_bus.mosi !=
    ...
    NRFSPI->begin(
        (int8_t)bruceConfig.NRF24_bus.sck,
        (int8_t)bruceConfig.NRF24_bus.miso,
        (int8_t)bruceConfig.NRF24_bus.mosi
    );
    delay(10);

    if (NRFradio.begin(
            NRFSPI, rf24_gpio_pin_t(bruceConfig.NRF24_bus.io0), rf24_gpio_pin_t(bruceConfig.NRF24_bus.cs)
        )) {
        return true;
    } else return false;
}
