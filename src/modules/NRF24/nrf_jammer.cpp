...
void nrf_jammer() {
    ...
    if (nrf_start(mode)) {
        ...
        drawMainBorder();
        if (CHECK_NRF_UART(mode)) NRFSerial.println("RADIOS");
        ...
        while (!check(SelPress)) {
            ...
            if (redraw) {
                ...
                if (CHECK_NRF_UART(mode)) {
                    if (NRFSerial.available()) { NRFSerial.println(modes[modeIndex].name); }
                }
                redraw = false;
                delay(200);
            }
        ...
        if (CHECK_NRF_UART(mode)) {
            if (NRFSerial.available()) { NRFSerial.println("OFF"); }
        }
    ...
    if (nrf_start(mode)) {
        ...
        while (!check(SelPress)) {
            ...
            if (redraw) {
                ...
                if (CHECK_NRF_UART(mode)) {
                    if (NRFSerial.available()) { NRFSerial.println("CH_" + String(channel)); }
                }
                tft.drawRoundRect(5, 5, tftWidth - 10, tftHeight - 10, 5, bruceConfig.priColor);
                redraw = false;
                delay(200);
