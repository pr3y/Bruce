...
bool copyToFs(FS from, FS to, String path, bool draw) {
  ...
  //tft.drawRect(5,TFT_HEIGHT-12, (TFT_WIDTH-10), 9, bruceConfig.priColor);
  while ((bytesRead = source.read(buff, bufSize)) > 0) {
    if (dest.write(buff, bytesRead) != bytesRead) {
      ...
      if(draw) tft.drawArc(TFT_WIDTH/2,TFT_HEIGHT/2,TFT_HEIGHT/4,TFT_HEIGHT/5,0,int(rad),ALCOLOR,bruceConfig.bgColor,true);
    }
  }
...
bool pasteFile(FS fs, String path) {
  ...
  while ((bytesRead = sourceFile.read(buff, bufSize)) > 0) {
    if (destFile.write(buff, bytesRead) != bytesRead) {
      ...
      prog+=bytesRead;
      float rad = 360*prog/tot;
      tft.drawArc(TFT_WIDTH/2,TFT_HEIGHT/2,TFT_HEIGHT/4,TFT_HEIGHT/5,0,int(rad),ALCOLOR,bruceConfig.bgColor,true);
      //tft.fillRect(7,TFT_HEIGHT-10, (TFT_WIDTH-14)*prog/tot, 5, bruceConfig.priColor);
    }
  }
...
void fileInfo(FS fs, String filepath) {
  ...
  padprintln("");
  tft.drawCentreString("-"+String(file.name()), TFT_WIDTH/2, tft.getCursorY(), 1);
  padprintln("\n");
