...
bool BruceTheme::openThemeFile(FS* fs, String filepath) {
    ...
    if(fs->exists(baseThemePath + _th["wifi"].as<String>()))        {theme.wifi = true;         theme.paths.wifi    = _th["wifi"].as<String>();        }else log_w("THEME: file not found: wifi");
    if(fs->exists(baseThemePath + _th["ble"].as<String>()))         {theme.ble = true;          theme.paths.ble     = _th["ble"].as<String>();         }else log_w("THEME: file not found: ble");
    if(fs->exists(baseThemePath + _th["rf"].as<String>()))          {theme.rf = true;           theme.paths.rf      = _th["rf"].as<String>();          }else log_w("THEME: file not found: rf");
    if(fs->exists(baseThemePath + _th["rfid"].as<String>()))        {theme.rfid = true;         theme.paths.rfid    = _th["rfid"].as<String>();        }else log_w("THEME: file not found: rfid");
    if(fs->exists(baseThemePath + _th["fm"].as<String>()))          {theme.fm = true;           theme.paths.fm      = _th["fm"].as<String>();          }else log_w("THEME: file not found: fm");
    if(fs->exists(baseThemePath + _th["ir"].as<String>()))          {theme.ir = true;           theme.paths.ir      = _th["ir"].as<String>();          }else log_w("THEME: file not found: ir");
    if(fs->exists(baseThemePath + _th["files"].as<String>()))       {theme.files = true;        theme.paths.files   = _th["files"].as<String>();       }else log_w("THEME: file not found: files");
    if(fs->exists(baseThemePath + _th["gps"].as<String>()))         {theme.gps = true;          theme.paths.gps     = _th["gps"].as<String>();         }else log_w("THEME: file not found: gps");
    if(fs->exists(baseThemePath + _th["nrf"].as<String>()))         {theme.nrf = true;          theme.paths.nrf     = _th["nrf"].as<String>();         }else log_w("THEME: file not found: nrf");
    if(fs->exists(baseThemePath + _th["interpreter"].as<String>())) {theme.interpreter = true;  theme.paths.interpreter = _th["interpreter"].as<String>(); }else log_w("THEME: file not found: interpreter");
    if(fs->exists(baseThemePath + _th["clock"].as<String>()))       {theme.clock = true;        theme.paths.clock   = _th["clock"].as<String>();       }else log_w("THEME: file not found: clock");
    if(fs->exists(baseThemePath + _th["others"].as<String>()))      {theme.others = true;       theme.paths.others  = _th["others"].as<String>();      }else log_w("THEME: file not found: others");
    if(fs->exists(baseThemePath + _th["connect"].as<String>()))     {theme.connect = true;      theme.paths.connect = _th["connect"].as<String>();     }else log_w("THEME: file not found: connect");
    if(fs->exists(baseThemePath + _th["config"].as<String>()))      {theme.config = true;       theme.paths.config  = _th["config"].as<String>();      }else log_w("THEME: file not found: config");
