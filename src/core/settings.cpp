...
void runClockLoop() {
  ...
  if(millis()-tmp>1000) {
    ...
    tft.drawRect(10, 10, TFT_WIDTH-15,TFT_HEIGHT-15, bruceConfig.priColor);
    tft.setCursor(64, TFT_HEIGHT/3+5);
    tft.setTextSize(4);
    #if defined(HAS_RTC)
      _rtc.GetBm8563Time();
      _rtc.GetTime(&_time);
    ...
    #else
      tft.drawCentreString(timeStr,TFT_WIDTH/2,TFT_HEIGHT/2-13,1);
    #endif
    tmp=millis();
  }
