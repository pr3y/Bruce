...
void startEvilPortal(String tssid, uint8_t channel, bool deauth) {
    ...
    while(1) {
      if(redraw) {
        ...
        tft.drawCentreString("Evil Portal",TFT_WIDTH/2, 29, SMOOTH_FONT);
        tft.setCursor(7,49);
        ...
        if (deauth){
          if (hold_deauth) {
            tft.setTextSize(FP);
            tft.setTextColor(bruceConfig.priColor);
            tft.drawRightString("Deauth OFF", TFT_WIDTH-7,TFT_HEIGHT-14,SMOOTH_FONT);
          } else {
            tft.setTextSize(FP);
            tft.setTextColor(TFT_RED);
            tft.drawRightString("Deauth ON", TFT_WIDTH-7,TFT_HEIGHT-14,SMOOTH_FONT);
          }
        }
