...
static SNSS_RETURN_CODE SNSS_ReadBlockHeader(SnssBlockHeader *header, SNSS_FILE *snssFile) {
    ...
    if (fread(headerBytes, 12, 1, snssFile->fp) != 1) { return SNSS_READ_FAILED; }
...
static SNSS_RETURN_CODE SNSS_WriteBlockHeader(SnssBlockHeader *header, SNSS_FILE *snssFile) {
    ...
    if (fwrite(headerBytes, 12, 1, snssFile->fp) != 1) { return SNSS_WRITE_FAILED; }
...
const char *SNSS_GetErrorString(SNSS_RETURN_CODE code) {
    switch (code) {
        case SNSS_OK: return "no error";

        case SNSS_BAD_FILE_TAG: return "not an SNSS file";

        case SNSS_OPEN_FAILED: return "could not open SNSS file";

        case SNSS_CLOSE_FAILED: return "could not close SNSS file";

        case SNSS_READ_FAILED: return "could not read from SNSS file";

        case SNSS_WRITE_FAILED: return "could not write to SNSS file";

        case SNSS_OUT_OF_MEMORY: return "out of memory";

        case SNSS_UNSUPPORTED_BLOCK: return "unsupported block type";
...
static SNSS_RETURN_CODE SNSS_ReadFileHeader(SNSS_FILE *snssFile) {
    if (fread(snssFile->headerBlock.tag, 4, 1, snssFile->fp) != 1) { return SNSS_READ_FAILED; }

    if (0 != strncmp(snssFile->headerBlock.tag, "SNSS", 5)) { return SNSS_BAD_FILE_TAG; }
...
static SNSS_RETURN_CODE SNSS_WriteFileHeader(SNSS_FILE *snssFile) {
    ...
    if (fwrite(writeBuffer, 8, 1, snssFile->fp) != 1) { return SNSS_WRITE_FAILED; }
...
SNSS_OpenFile(SNSS_FILE **snssFile, const char *filename, SNSS_OPEN_MODE mode) {
    if (!snssFileBlock) { snssFileBlock = (SNSS_FILE *)NOFRENDO_MALLOC(sizeof(SNSS_FILE)); }
    *snssFile = snssFileBlock;
    if (NULL == *snssFile) { return SNSS_OUT_OF_MEMORY; }
    ...
    if (NULL == (*snssFile)->fp) { return SNSS_OPEN_FAILED; }
...
SNSS_CloseFile(SNSS_FILE **snssFile) {
    ...
    if (NULL == *snssFile) { return SNSS_OK; }
    ...
    if (SNSS_OPEN_WRITE == (*snssFile)->mode) {
        ...
        if (SNSS_OK != (code = SNSS_WriteFileHeader(*snssFile))) { return SNSS_CLOSE_FAILED; }

        fseek((*snssFile)->fp, prevLoc, SEEK_SET);
    }

    if (fclose((*snssFile)->fp) != 0) { return SNSS_CLOSE_FAILED; }
...
SNSS_GetNextBlockType(SNSS_BLOCK_TYPE *blockType, SNSS_FILE *snssFile) {
    ...
    if (fread(tagBuffer, TAG_LENGTH, 1, snssFile->fp) != 1) { return SNSS_READ_FAILED; }
    tagBuffer[TAG_LENGTH] = '\0';

    /* reset the file pointer to the start of the block */
    if (fseek(snssFile->fp, -TAG_LENGTH, SEEK_CUR) != 0) { return SNSS_READ_FAILED; }
...
SNSS_SkipNextBlock(SNSS_FILE *snssFile) {
    ...
    if (fseek(snssFile->fp, TAG_LENGTH + sizeof(unsigned int), SEEK_CUR) != 0) { return SNSS_READ_FAILED; }

    /* get the block data length */
    if (fread(&blockLength, sizeof(unsigned int), 1, snssFile->fp) != 1) { return SNSS_READ_FAILED; }
    blockLength = swap32(blockLength);

    /* skip over the block data */
    if (fseek(snssFile->fp, blockLength, SEEK_CUR) != 0) { return SNSS_READ_FAILED; }
...
static SNSS_RETURN_CODE SNSS_ReadBaseBlock(SNSS_FILE *snssFile) {
    if (!blockBytes) { blockBytes = (char *)NOFRENDO_MALLOC(BASE_BLOCK_LENGTH); }
    SnssBlockHeader header;

    if (SNSS_ReadBlockHeader(&header, snssFile) != SNSS_OK) { return SNSS_READ_FAILED; }
...
static SNSS_RETURN_CODE SNSS_WriteBaseBlock(SNSS_FILE *snssFile) {
    ...
    if ((returnCode = SNSS_WriteBlockHeader(&header, snssFile)) != SNSS_OK) { return returnCode; }
    ...
    if (fwrite(blockBytes, BASE_BLOCK_LENGTH, 1, snssFile->fp) != 1) { return SNSS_WRITE_FAILED; }
...
static SNSS_RETURN_CODE SNSS_ReadSramBlock(SNSS_FILE *snssFile) {
    ...
    if (fread(&snssFile->sramBlock.sramEnabled, 1, 1, snssFile->fp) != 1) { return SNSS_READ_FAILED; }
...
static SNSS_RETURN_CODE SNSS_WriteSramBlock(SNSS_FILE *snssFile) {
    ...
    if (fwrite(&snssFile->sramBlock.sramEnabled, 1, 1, snssFile->fp) != 1) { return SNSS_WRITE_FAILED; }
...
static SNSS_RETURN_CODE SNSS_ReadMapperBlock(SNSS_FILE *snssFile) {
    ...
    if (!blockBytes) { return SNSS_OUT_OF_MEMORY; }
...
static SNSS_RETURN_CODE SNSS_WriteMapperBlock(SNSS_FILE *snssFile) {
    ...
    if (fwrite(blockBytes, MAPPER_BLOCK_LENGTH, 1, snssFile->fp) != 1) { return SNSS_WRITE_FAILED; }
...
SNSS_ReadBlock(SNSS_FILE *snssFile, SNSS_BLOCK_TYPE blockType) {
    switch (blockType) {
        ...
        default: return SNSS_UNSUPPORTED_BLOCK;
    }
}
