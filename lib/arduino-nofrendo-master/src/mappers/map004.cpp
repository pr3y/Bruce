...
static void map4_write(uint32 address, uint8 value) {
    switch (address & 0xE001) {
        case 0x8000:
            ...
            if (reg != (value & 0x40)) {
                if (value & 0x40) mmc_bankrom(8, 0x8000, (mmc_getinfo()->rom_banks * 2) - 2);
                else mmc_bankrom(8, 0xC000, (mmc_getinfo()->rom_banks * 2) - 2);
            }
            reg = value & 0x40;
            break;
        ...
        case 0x8001:
            switch (command & 0x07) {
                ...
                case 6: mmc_bankrom(8, (command & 0x40) ? 0xC000 : 0x8000, value); break;
        ...
        case 0xA000:
            ...
            if (0 == (mmc_getinfo()->flags & ROM_FLAG_FOURSCREEN)) {
                if (value & 1) ppu_mirror(0, 0, 1, 1); /* horizontal */
