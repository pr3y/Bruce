    ...
    {                                                                                                        \
        if ((value) < 0xE0)                                                                                  \
            ppu_setpage(                                                                                     \
                1,                                                                                           \
                (table) + 8,                                                                                 \
                &mmc_getinfo()->vrom[((value) % (mmc_getinfo()->vrom_banks * 8)) << 10] -                    \
                    (0x2000 + ((table) << 10))                                                               \
            );                                                                                               \
        else                                                                                                 \
            ppu_setpage(                                                                                     \
                1, (table) + 8, &mmc_getinfo()->vram[((value) & 7) << 10] - (0x2000 + ((table) << 10))       \
            );                                                                                               \
        ppu_mirrorhipages();                                                                                 \
    }
...
static void map19_write(uint32 address, uint8 value) {
    ...
    switch (reg) {
        ...
        case 0x15:
        case 0x16:
        case 0x17: mmc_bankvrom(1, (reg & 7) << 10, value); break;

        case 0x18:
        case 0x19:
        case 0x1A:
        case 0x1B: N_BANK1(reg & 3, value); break;

        case 0x1C: mmc_bankrom(8, 0x8000, value); break;

        case 0x1D: mmc_bankrom(8, 0xA000, value); break;

        case 0x1E: mmc_bankrom(8, 0xC000, value); break;
...
static void map19_setstate(SnssMapperBlock *state) {
    irq.counter =
        (state->extraData.mapper19.irqCounterHighByte << 8) | state->extraData.mapper19.irqCounterLowByte;
