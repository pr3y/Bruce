...
#define VRC_PBANK(bank, value, high)                                                                         \
    do {                                                                                                     \
        if ((high)) highprgnybbles[(bank)] = (value) & 0x0F;                                                 \
        else lowprgnybbles[(bank)] = (value) & 0x0F;                                                         \
...
static void map18_write(uint32 address, uint8 value) {
    switch (address) {
        ...
        case 0xE000:
            ...
            irq.clockticks =
                (irq.nybbles[0]) | (irq.nybbles[1] << 4) | (irq.nybbles[2] << 8) | (irq.nybbles[3] << 12);
        ...
        case 0xF000:
            ...
            break;
        case 0xF001: irq.enabled = value & 0x01; break;
        case 0xF002:
            switch (value & 0x03) {
...
static void map18_setstate(SnssMapperBlock *state) {
    irq.counter =
        (state->extraData.mapper18.irqCounterHighByte << 8) | state->extraData.mapper18.irqCounterLowByte;
