...
static void map16_write(uint32 address, uint8 value) {
    ...
    if (reg < 8) {
        ...
        switch (address & 0x000F) {
            case 0x8: mmc_bankrom(16, 0x8000, value); break;
            ...
            case 0xA: irq.enabled = (value & 1) ? true : false; break;

            case 0xB: irq.counter = (irq.counter & 0xFF00) | value; break;

            case 0xC: irq.counter = (value << 8) | (irq.counter & 0xFF); break;
...
static void map16_setstate(SnssMapperBlock *state) {
    irq.counter =
        (state->extraData.mapper16.irqCounterHighByte << 8) | state->extraData.mapper16.irqCounterLowByte;
