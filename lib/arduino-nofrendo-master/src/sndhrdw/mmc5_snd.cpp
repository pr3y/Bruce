...
static int32 mmc5_rectangle(mmc5rectangle_t *chan) {
    ...
    */
    ...
    while (chan->env_phase < 0) {
        ...
        if (chan->holdnote) chan->env_vol = (chan->env_vol + 1) & 0x0F;
        else if (chan->env_vol < 0x0F) chan->env_vol++;
    }
    ...
    if (chan->fixed_envelope) output = chan->volume << 8; /* fixed volume */
    else output = (chan->env_vol ^ 0x0F) << 8;
#endif
...
#ifdef APU_OVERSAMPLE
        if (chan->adder < chan->duty_flip) total += output;
        else total -= output;
    ...
    if (0 == chan->adder) chan->output_vol = output;
    else if (chan->adder == chan->duty_flip) chan->output_vol = -output;
#endif
...
static uint8 mmc5_read(uint32 address) {
    ...
    switch (address) {
        case 0x5205: return (uint8)retval;

        case 0x5206: return (uint8)(retval >> 8);
...
static void mmc5_write(uint32 address, uint8 value) {
    ...
    switch (address) {
        ...
        case 0x5010:
            if (value & 0x01) mmc5.dac.enabled = true;
            else mmc5.dac.enabled = false;
            break;

        case 0x5011: mmc5.dac.output = (value ^ 0x80) << 8; break;

        case 0x5205: mmc5.mul[0] = value; break;

        case 0x5206: mmc5.mul[1] = value; break;
