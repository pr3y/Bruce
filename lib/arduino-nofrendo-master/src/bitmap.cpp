...
static bitmap_t *_make_bitmap(uint8 *data_addr, bool hw, int width, int height, int pitch, int overdraw) {
    ...
    bitmap = (bitmap_t *)NOFRENDO_MALLOC(sizeof(bitmap_t) + (sizeof(uint8 *) * height));
...
bitmap_t *bmp_create(int width, int height, int overdraw) {
    ...
    addr =
        (uint8 *)NOFRENDO_MALLOC(((pitch * height) + 3) & 0xFFFFFFF8); /* add max 32-bit aligned adjustment */
...
void bmp_destroy(bitmap_t **bitmap) {
    if (*bitmap) {
        if ((*bitmap)->data && false == (*bitmap)->hardware) NOFRENDO_FREE((*bitmap)->data);
