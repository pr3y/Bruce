...
File *_fopen(const char *path, const char *mode) {
    ...
    File tmp = LittleFS.open(path, mode);
...
#define _fread(B, N, L, F) F->read((uint8_t *)(B), (N) * (L))
#endif
...
static int rom_allocsram(rominfo_t *rominfo) {
    /* Load up SRAM */
    rominfo->sram = (uint8_t *)NOFRENDO_MALLOC(SRAM_BANK_LENGTH * rominfo->sram_banks);
...
static int rom_loadrom(File *fp, rominfo_t *rominfo) {
    ...
    rominfo->rom = (uint8_t *)mem_alloc(rominfo->rom_banks * ROM_BANK_LENGTH, false);
    if (NULL == rominfo->rom) {
    ...
    if (rominfo->vrom_banks) {
        ...
        rominfo->vrom = (uint8_t *)mem_alloc(rominfo->vrom_banks * VROM_BANK_LENGTH, false);
        ...
        rominfo->vram = (uint8_t *)NOFRENDO_MALLOC(VRAM_LENGTH);
...
static void rom_checkforpal(rominfo_t *rominfo, ppu_t *ppu) {
    ...
    if (NULL == fp) {
        nofrendo_log_printf("no palette found\n");
...
char *rom_getinfo(rominfo_t *rominfo) {
    ...
    else strncpy(romname, rominfo->filename, PATH_MAX);
...
rominfo_t *rom_load(const char *filename, ppu_t *ppu) {
    ...
    rominfo = (rominfo_t *)NOFRENDO_MALLOC(sizeof(rominfo_t));
    ...
    if (NULL == fp) gui_sendmsg(GUI_RED, "%s not found, will use default ROM", filename);

    /* Get the header and stick it into rominfo struct */
    if (NULL == fp) intro_get_header(rominfo);
    else if (rom_getheader(fp, rominfo)) goto _fail;
    ...
    */
    ...
    if (NULL != fp) rom_loadtrainer(fp, rominfo);

    if (NULL == fp) {
        if (intro_get_rom(rominfo)) goto _fail;
    } else if (rom_loadrom(fp, rominfo)) goto _fail;

    /* Close the file */
    if (NULL != fp) _fclose(fp);
...
void rom_free(rominfo_t **rominfo) {
    ...
    if ((*rominfo)->sram) NOFRENDO_FREE((*rominfo)->sram);
    if ((*rominfo)->rom) NOFRENDO_FREE((*rominfo)->rom);
    if ((*rominfo)->vrom) NOFRENDO_FREE((*rominfo)->vrom);
    if ((*rominfo)->vram) NOFRENDO_FREE((*rominfo)->vram);
