...
ppu_t *ppu_create(void) {
    ...
    temp = (ppu_t *)NOFRENDO_MALLOC(sizeof(ppu_t));
...
void ppu_setpage(int size, int page_num, uint8 *location) {
    ...
    switch (size) {
        case 8:
            ...
            ppu.page[page_num++] = location;
            ppu.page[page_num++] = location;
        case 4: ppu.page[page_num++] = location; ppu.page[page_num++] = location;
        case 2: ppu.page[page_num++] = location;
        case 1: ppu.page[page_num++] = location; break;
    }
}
...
static void ppu_oamdma(uint8 value) {
    ...
    do { ppu.oam[oam_loc++] = nes6502_getbyte(cpu_address++); } while (oam_loc != ppu.oam_addr);
...
void ppu_writehigh(uint32 address, uint8 value) {
    switch (address) {
        case PPU_OAMDMA: ppu_oamdma(value); break;
        ...
        case PPU_JOY1: /* frame IRQ control */ nes_setfiq(value); break;
...
uint8 ppu_readhigh(uint32 address) {
    ...
    switch (address) {
        case PPU_JOY0: value = input_get(INP_JOYPAD0); break;

        case PPU_JOY1:
            /* TODO: better input handling */
            value = input_get(
                INP_ZAPPER | INP_JOYPAD1
                /*| INP_ARKANOID*/
                /*| INP_POWERPAD*/
            );
            break;
...
void ppu_write(uint32 address, uint8 value) {
    ...
    switch (address & 0x2007) {
        ...
        case PPU_OAMADDR: ppu.oam_addr = value; break;

        case PPU_OAMDATA: ppu.oam[ppu.oam_addr++] = value; break;
...
INLINE int
draw_oamtile(uint8 *surface, uint8 attrib, uint8 pat1, uint8 pat2, const uint8 *col_tbl, bool check_strike) {
    int strike_pixel = -1;
...
static void ppu_renderoam(uint8 *vidbuf, int scanline) {
    ...
    for (sprite_num = 0; sprite_num < 64; sprite_num++, sprite_ptr++) {
        ...
        if ((sprite_y > scanline) || (sprite_y <= (scanline - sprite_height)) || (0 == sprite_y) ||
            (sprite_y >= 240))
            continue;
        ...
        */
        ...
        strike_pixel = draw_oamtile(
            bmp_ptr, attrib, data_ptr[0], data_ptr[8], ppu.palette + 16 + col_high, check_strike
        );
...
INLINE void draw_deadsprite(bitmap_t *bmp, int x, int y, int height) {
    ...
    uint8 *vid;
    uint8 colbuf[8] = {
        GUI_BLACK, GUI_BLACK, GUI_BLACK, GUI_BLACK, GUI_BLACK, GUI_BLACK, GUI_BLACK, GUI_DKGRAY
    };
...
static void draw_sprite(bitmap_t *bmp, int x, int y, uint8 tile_num, uint8 attrib) {
    ...
    else vram_adr = ppu.obj_base + (tile_num << 4);
...
void ppu_dumpoam(bitmap_t *bmp, int x_loc, int y_loc) {
    ...
    for (sprite = 0; sprite < 64; sprite++) {
        ...
        if (height == 16) y_pos = (sprite & 0xF0) + (sprite >> 4) + y_loc;
        else y_pos = ((sprite & 0xF0) >> 1) + (sprite >> 4) + y_loc;
        ...
        else draw_deadsprite(bmp, x_pos + 1, y_pos + 1, height);
