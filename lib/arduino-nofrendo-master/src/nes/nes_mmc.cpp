...
void mmc_bankvrom(int size, uint32 address, int bank) {
    ...
    switch (size) {
        ...
        default: nofrendo_log_printf("invalid VROM bank size %d\n", size);
    }
}
...
void mmc_bankrom(int size, uint32 address, int bank) {
    ...
    switch (size) {
        ...
        default: nofrendo_log_printf("invalid ROM bank size %d\n", size); break;
    }
...
static void mmc_setpages(void) {
    ...
    if (mmc.cart->flags & ROM_FLAG_FOURSCREEN) {
        ...
        if (MIRROR_VERT == mmc.cart->mirror) ppu_mirror(0, 1, 0, 1);
        else ppu_mirror(0, 0, 1, 1);
    }
...
void mmc_destroy(mmc_t **nes_mmc) {
    if (*nes_mmc) NOFRENDO_FREE(*nes_mmc);
}
...
mmc_t *mmc_create(rominfo_t *rominfo) {
    ...
    temp = (mmc_t *)NOFRENDO_MALLOC(sizeof(mmc_t));
