...
#define LAST_MEMORY_HANDLER {0xFFFFFFFF, 0xFFFFFFFF, NULL}
    ...
    if (intf->sound_ext) {
        if (NULL != intf->sound_ext->mem_read) {
            for (count = 0; num_handlers < MAX_MEM_HANDLERS; count++, num_handlers++) {
                ...
                memcpy(
                    &machine->readhandler[num_handlers],
                    &intf->sound_ext->mem_read[count],
    ...
    if (intf->sound_ext) {
        if (NULL != intf->sound_ext->mem_write) {
            for (count = 0; num_handlers < MAX_MEM_HANDLERS; count++, num_handlers++) {
                ...
                memcpy(
                    &machine->writehandler[num_handlers],
                    &intf->sound_ext->mem_write[count],
...
static void system_video(bool draw) {
    ...
    vid_blit(
        nes.vidbuf,
        0,
        (NES_SCREEN_HEIGHT - NES_VISIBLE_HEIGHT) / 2,
        0,
        0,
        NES_SCREEN_WIDTH,
        NES_VISIBLE_HEIGHT
...
void nes_destroy(nes_t **machine) {
    if (*machine) {
        ...
        if ((*machine)->cpu) {
            if ((*machine)->cpu->mem_page[0]) NOFRENDO_FREE((*machine)->cpu->mem_page[0]);
...
int nes_insertcart(const char *filename, nes_t *machine) {
    ...
    if (NULL != machine->rominfo->vram) machine->ppu->vram_present = true;
...
nes_t *nes_create(void) {
    ...
    machine = (nes_s *)NOFRENDO_MALLOC(sizeof(nes_t));
    ...
    if (NULL == machine->vidbuf) goto _fail;
#endif /* NOFRENDO_DOUBLE_FRAMEBUFFER */

    machine->autoframeskip = true;

    /* cpu */
    machine->cpu = (nes6502_context *)NOFRENDO_MALLOC(sizeof(nes6502_context));
    if (NULL == machine->cpu) goto _fail;

    memset(machine->cpu, 0, sizeof(nes6502_context));

    /* allocate 2kB RAM */
    machine->cpu->mem_page[0] = (uint8 *)NOFRENDO_MALLOC(NES_RAMSIZE);
