...
static void my_destroy(myvar_t **var) {
    ...
    if ((*var)->group) NOFRENDO_FREE((*var)->group);
    if ((*var)->key) NOFRENDO_FREE((*var)->key);
    if ((*var)->value) NOFRENDO_FREE((*var)->value);
    NOFRENDO_FREE(*var);
}
...
static myvar_t *my_create(const char *group, const char *key, const char *value) {
    ...
    var = (myvar_t *)NOFRENDO_MALLOC(sizeof(*var));
    if (NULL == var) { return 0; }

    var->less = var->greater = NULL;
    var->group = var->key = var->value = NULL;

    if ((var->group = (char *)NOFRENDO_MALLOC(strlen(group) + 1)) &&
        (var->key = (char *)NOFRENDO_MALLOC(strlen(key) + 1)) &&
        (var->value = (char *)NOFRENDO_MALLOC(strlen(value) + 1))) {
...
static myvar_t *my_lookup(const char *group, const char *key) {
    ...
    while (current && ((cmp = stricmp(group, current->group)) || (cmp = stricmp(key, current->key)))) {
        if (cmp < 0) current = current->less;
        else current = current->greater;
    }
...
static char *my_getline(FILE *stream) {
    ...
    do {
        if (NULL == (fgets(buf, sizeof(buf), stream))) {
            if (dynamic) NOFRENDO_FREE(dynamic);
            return 0;
        }

        if (NULL == dynamic) {
            dynamic = (char *)NOFRENDO_MALLOC(strlen(buf) + 1);
            if (NULL == dynamic) { return 0; }
            strcpy(dynamic, buf);
        } else {
            /* a mini-version of realloc that works with our memory manager */
            char *temp = NULL;
            temp = (char *)NOFRENDO_MALLOC(strlen(dynamic) + strlen(buf) + 1);
            ...
            do {
                ...
                switch (*s) {
                    ...
                    case '[':
                        ...
                        if ((value = (char *)NOFRENDO_MALLOC(strlen(group) + 1))) { strcpy(value, group); }
                        group = value;
                        break;
                    ...
                    default:
                        ...
                        while (strlen(key) && isspace(key[strlen(key) - 1])) key[strlen(key) - 1] = '\0';
...
static void close_config(void) {
    if (true == mySaveNeeded) { save_config(config.filename); }
