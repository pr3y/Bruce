...
static void mem_init(void) {
    ...
    mem_record = (memblock_t *)mem_alloc(MAX_BLOCKS * sizeof(memblock_t), false);
    ...
    for (i = 0; i < MAX_BLOCKS; i++) {
        if (data == mem_record[i].block_addr) {
            if (mem_checkguardblock(mem_record[i].block_addr, GUARD_LENGTH)) {
                sprintf(
                    fail, "mem_deleteblock 0x%08X at line %d of %s -- block corrupt", (uint32)data, line, file
                );
                ASSERT_MSG(fail);
            }
...
void *_my_malloc(int size, char *file, int line) {
    ...
    if (false != mem_debug) temp = mem_guardalloc(size, GUARD_LENGTH);
    else
        // temp = malloc(size);
...
void _my_free(void **data, char *file, int line) {
    ...
    if (0 == mem_blockcount) { ASSERT_MSG("free: attempted to free memory when no blocks available"); }
...
void mem_checkleaks(void) {
    ...
    } else nofrendo_log_printf("no memory leaks\n");
#endif /* NOFRENDO_DEBUG */
}
...
void mem_checkblocks(void) {
    ...
    for (i = 0; i < MAX_BLOCKS; i++) {
        if (mem_record[i].block_addr) {
            if (mem_checkguardblock(mem_record[i].block_addr, GUARD_LENGTH)) {
                nofrendo_log_printf(
                    "addr: 0x%08X, size: %d, line %d of %s -- block corrupt\n",
                    (uint32)mem_record[i].block_addr,
                    mem_record[i].block_size,
                    mem_record[i].line_num,
                    mem_record[i].file_name
