...
static const RCSwitch::Protocol PROGMEM proto[] = {
  ...
  // from https://github.com/sui77/rc-switch/pull/464
...
void RCSwitch::transmit(HighLow pulses) {
  ...
  // from: https://github.com/sui77/rc-switch/pull/432
...
bool RCSwitch::RAWavailable() {
  bool result=false;
  if((millis() - RCSwitch::_lastMicros)>100000 && RCSwitch::RAWtransitions > 10) result=true;

  return result;
}

void RCSwitch::resetAvailable() {
  // Clear timings when clearing signals
  for(int i=0; i<RCSWITCH_RAW_MAX_CHANGES; i++) RCSwitch::RAWtimings[i]=0;
...
void RECEIVE_ATTR RCSwitch::handleInterrupt() {
  ...
  if(changeRAWCount>RCSWITCH_RAW_MAX_CHANGES || duration>80000) changeRAWCount = 0;
  RCSwitch::RAWtimings[changeRAWCount++] = duration;
  RCSwitch::timings[changeCount++] = duration;

  if(changeRAWCount==10) {
    RCSwitch::_lastMicros=micros();
    RCSwitch::RAWtransitions=11; // arbitrary
  }
  lastTime = time;  
}
#endif
